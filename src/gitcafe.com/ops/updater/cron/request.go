...
func BuildcmdbhbRequest(hostname string,vminfo string, cpuinfo string) model.CmdbhbRequest {
	req := model.CmdbhbRequest{Hostname: hostname}
	
	cmdbAgents:=[]*model.CmdbAgent{}
	now:= time.Now().Unix()
        
	cmdbAgent:=&model.CmdbAgent{
		Vminfo:  vminfo,
		Cpuinfo: cpuinfo,
		Timestamp: now,
		}
	
	cmdbAgents=append(cmdbAgents,cmdbAgent)
	
	req.CmdbAgents = cmdbAgents
        return req
}
